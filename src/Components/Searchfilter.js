            ...
            <div className="container">
                    ...
                    <div className='table_top'>
                        <div className='table_top_a'>
                            <div className='table_top_sort'>
                                <label htmlFor="sortby" className='label_sort'>Sort by</label>
                            ...
                            <div className='table_top_time'>
                                <label htmlFor="time" className='label_time'>Time</label>
                        ...
                        <div className='table_top_search'>
                            ...
                            />
                            <div className='icon_search'><i class="fa fa-search searchicon" aria-hidden="true"></i></div>
                        </div>
                    </div>
                </div>
                ...
                <div className="media_table">
                        ...
                        <table class="table table-hover  media_table" >
                            <thead>
                                <tr>
                                    ...
                                    <th className='head_daily'><button
                                        type="button"
                                    ...
                                    <th className='head_weekly'><button
                                        type="button"
                                    ...
                                    <th className='head_monthy'> <button
                                        type="button"
                                    ...
                                    <th className='head_alltime'><button
                                        type="button"
                            ...
                            <tbody>
                                {items.slice(
                                    ...
                                    return (
                                        ...
                                        <tr key={index}>
                                            <td className='body_sno'>{currentPage<=1 ? index+1 : ((index+1)+(ITEMS_PER_PAGE*(currentPage-1)))}</td>

                                            <td className='body_trader'><img src={items.picture} alt="" className='trader_img' />{items.trader}</td>

                                            <td className= 'body_profitloss'><h6 className= {items.daily > 0 ? '' : 'color_red '}>{items.daily > 0 ? `+${(items.daily).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.daily).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${(items.dailyA).toLocaleString('en-US', {valute: 'USD'})}`}</p></td>

                                            <td className='body_trades'><h6 className= {items.weekly > 0 ? '' : 'color_red'}>{items.weekly > 0 ? `+${(items.weekly).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.weekly).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${(items.weeklyA).toLocaleString('en-US', {valute: 'USD'})}`}</p></td>

                                            <td className='body_traderequity color_red'><h6 className= {items.monthly  > 0 ? '' : 'color_red'}>{items.monthly > 0 ? `+${(items.monthly).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.monthly).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${(items.monthlyA).toLocaleString('en-US', {valute: 'USD'})}`}</p></td>

                                            <td className='body_followerequity'><h6 className= {items.alltime > 0 ? '' : 'color_red'}>{items.alltime > 0 ? `+${(items.alltime).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.alltime).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${(items.alltimeA).toLocaleString('en-US', {valute: 'USD'})}`}</p></td>

                                            <td className='body_follower'>{`${(items.follower).toLocaleString('en-US', {valute: 'USD'})}`}</td>

                                            <td className='body_openposition'>{items.openposition}</td>

                                            <td className='body_follow'><input type="button" value="+ Follow" className='btn btn-primary' onClick={(e) => {
                    ...
                    <div className='pagination_btm'>
                        <div className='btm_p'>
                            <p>Showing {currentPage<=1 ? (`1 - ${a}`) : (`${(1+((currentPage - 1)*ITEMS_PER_PAGE))} - ${((a)+(ITEMS_PER_PAGE*(currentPage-1)))}`)} of {items.length}</p>
                        </div>
                        <div >
                            <Pagination
                                    ...
                                    <div className="flex_body">
                                        <div key={index} className="media_content">
                                            <div className='firstLine'>
                                                ...
                                                <div className='line_a' onClick={() => requestSort('trader')}><img src={items.picture} alt="" className='trader_img' /><span>{items.trader}</span></div>
                                                <div className='firstline_a'>
                                                    <h6 onClick={() => requestSort('follower')}>Follower</h6>
                                                    <p>{(items.follower).toLocaleString('en-US', {valute: 'USD'})}</p>
                                                </div>
                                                <div className='firstline_b'>
                                                    <h6 onClick={() => requestSort('openposition')}>Open Position</h6>
                                                    <p>{items.openposition}</p>
                                                </div>
                                                <div className="media_btn"><input type="button" value="+" onClick={(e) => {
                                                    if (e.target.value == "-") {
                                                        e.target.value = "+"
                                            ...
                                            <div className='secondline'>
                                                <div className='secondline_a'>
                                                    ...
                                                    <div><h6 className= {items.daily > 0 ? '' : 'color_red '}>{items.daily > 0 ? `+${(items.daily).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.daily).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${items.dailyA}`}</p></div>
                                                </div>
                                                <div className='secondline_b'>
                                                    <h5 onClick={() => requestSort('weekly')}>Weekly</h5>
                                                    <div><h6 className= {items.weekly > 0 ? '' : 'color_red '}>{items.weekly > 0 ? `+${(items.weekly).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.weekly).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${items.weeklyA}`}</p></div>
                                                </div>
                                                <div className='secondline_c'>
                                                    <h5 onClick={() => requestSort('monthly')} >Monthly</h5>
                                                    <div><h6 className= {items.monthly > 0 ? '' : 'color_red '}>{items.monthly > 0 ? `+${(items.monthly).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.monthly).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${items.monthlyA}`}</p></div>
                                                </div>
                                                <div className='secondline_d'>
                                                    <h5 onClick={() => requestSort('alltime')} >All-time</h5>
                                                    <div><h6 className= {items.alltime > 0 ? '' : 'color_red '}>{items.alltime > 0 ? `+${(items.alltime).toLocaleString('en-US', {valute: 'USD'})}%` : `${(items.alltime).toLocaleString('en-US', {valute: 'USD'})}%`}</h6><p>{`≈ $${items.alltimeA}`}</p></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
